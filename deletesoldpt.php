...
$pp=$_POST["t2"];
$cs_id=$_POST["t3"];
$Query2="select count(*) from sold_pets where sd_id='$id' AND pet_id='$pp' AND cs_id='$cs_id'";
$Execute = mysqli_query($conn,$Query2);
$count = mysqli_fetch_row($Execute);
if($count[0]==1)
{
    $sql = "DELETE FROM sold_pets WHERE sd_id='$id' AND pet_id='$pp' AND cs_id='$cs_id'";
    if ($conn->query($sql) == TRUE) {
        echo '<div>
        <h1 style="color:#404040;font-size:30px; font-family: "Roboto", sans-serif;margin:auto;">Data deleted successfully</h1>
           </div>';
           $conn->query("call calculations_for_soldpets('$pp','$id','$cs_id')");
    } else {
...
border: 2px solid black;background-color: rgb(152, 116, 112);color:#f2f2f2;font-size:15px;"formaction="soldpets.php">back</button>
</form>
</body>
</html>